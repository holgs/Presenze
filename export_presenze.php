...
header('Content-Disposition: attachment; filename=export-presenze-'.date('Y-m-d').'.xls');  
...
$fname = "export-presenze-".date("Y-m-d").".csv";
if(isset($_GET['id']))
{
$id = $_GET['id'];
$query = "SELECT 
					...
					WHERE athl_pres_athl_id = $id
					ORDER BY athl_pres_date ASC";  
